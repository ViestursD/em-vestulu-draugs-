...
/**
 * ✅ Droša pick_decision_rule izsaukšana ar TOP kandidātiem, ja tava funkcija atbalsta 7. parametru ($top)
 * - ja neatbalsta, tomēr strādās (noķers kļūdu un atkārtos bez $top)
 */
function pick_rule_safe(
  PDO $pdo,
  string $buvesTips,
  string $buvesGrupa,
  string $objekts,
  ...
  $topOut = [];
  try {
    // mēģinam ar TOP kandidātu masīvu (ja funkcija atbalsta)
    return pick_decision_rule($pdo, $buvesTips, $buvesGrupa, $objekts, $darbiba, $kult, $topOut);
  } catch (Throwable $e) {
    // fallback: vecā signatūra bez topOut
...
$pinned = '';
$decision = null;        // lieto, kad kult != '*'
$decisionNo = null;      // lieto, kad kult == '*'
$decisionYes = null;     // lieto, kad kult == '*'
...
if ($isNew && $topic === 'ieceres_dokumentacija') {
  ...
  // ✅ GALVENĀ IZMAIŅA:
  // ja kult='*' -> atrast ABUS: kult=no un kult=yes, un rādīt A/B
  if ($kult === '*') {

    $topNo = [];
    $topYes = [];

    $decisionNo  = pick_rule_safe($pdo, $buvesTipsN, (string)$buvesGrupa, $objekts ?: '*', $darbibaN ?: '*', 'no',  $topNo);
    $decisionYes = pick_rule_safe($pdo, $buvesTipsN, (string)$buvesGrupa, $objekts ?: '*', $darbibaN ?: '*', 'yes', $topYes);

    $debugAdd("DB kandidāti (TOP 10) — kult=no", $topNo);
    $debugAdd("DB kandidāti (TOP 10) — kult=yes", $topYes);
    $debugAdd("DB lēmums (kult=no)", $decisionNo);
    $debugAdd("DB lēmums (kult=yes)", $decisionYes);

    if (!$decisionNo && !$decisionYes) {
      header("Location: " . APP_BASE . "/?err=" . urlencode("Nav atrasts BIS lēmums ne kult=no, ne kult=yes (embpd_decision_rules)."));
      exit;
    }
...
if (($decision === null && $decisionNo === null && $decisionYes === null) && $topic === 'ieceres_dokumentacija' && $pinned !== '') {
  $bt = ''; $bg = ''; $ob = ''; $dv = ''; $kp = '';
  foreach (explode("\n", $pinned) as $line) {
    $line = trim($line);
  ...
  if ($bt !== '' && $bg !== '' && $ob !== '' && $dv !== '') {
    ...
    if ($kp === '*') {
      ...
      $decisionNo  = pick_rule_safe($pdo, $bt, $bg, $ob ?: '*', $dv ?: '*', 'no',  $topNo2);
      $decisionYes = pick_rule_safe($pdo, $bt, $bg, $ob ?: '*', $dv ?: '*', 'yes', $topYes2);
      ...
      foreach ([$decisionNo, $decisionYes] as $dec) {
        ...
        if ($nf !== '' && !in_array($nf, $selected, true)) {
          $selected[] = $nf;
          $debugAdd("Turpinājums: pievienots DB normatīvais fails (A/B)", $nf);
        }
      }
      $selected = array_values(array_unique($selected));
      ...
      if ($decision) {
        ...
        if ($nf !== '' && !in_array($nf, $selected, true)) {
          $selected[] = $nf;
          $selected = array_values(array_unique($selected));
          $debugAdd("Turpinājums: pievienots DB normatīvais fails", $nf);
        }
      }
    }
  }
...
// ✅ PIEPRASĪT 2 SCENĀRIJUS, ja kult='*'
if ($kult === '*' && ($decisionNo || $decisionYes)) {
  $docNo  = (string)($decisionNo['doc_type'] ?? '');
  $docYes = (string)($decisionYes['doc_type'] ?? '');

  $extraRules .= "- Lietotājs kultūras pieminekli NAV norādījis (kult='*'), tāpēc atbildē OBLIGĀTI jāsniedz 2 scenāriji:\n";
  $extraRules .= "  A) Ja NAV kultūras piemineklis (kult=no) → jāiesniedz: {$docNo}.\n";
  $extraRules .= "  B) Ja IR kultūras piemineklis (kult=yes) → jāiesniedz: {$docYes}.\n";
  $extraRules .= "- Nekādā gadījumā nedrīkst piedāvāt citus dokumentu veidus ārpus A/B.\n";
...
// ✅ Rādām vai nu 1 lēmumu, vai A/B (kult='*')
if ($kult === '*' && ($decisionNo || $decisionYes)) {
  $contextBlock .= "BIS LĒMUMS (no DB) — DIVI SCENĀRIJI (jo kult='*'):\n";

  if ($decisionNo) {
    $contextBlock .= "\nA) Ja NAV kultūras piemineklis (kult=no):\n";
    ...
    $contextBlock .= "\nA) kult=no: DB ieraksts nav atrasts.\n";
  }

  if ($decisionYes) {
    $contextBlock .= "\nB) Ja IR kultūras piemineklis (kult=yes):\n";
    ...
    $contextBlock .= "\nB) kult=yes: DB ieraksts nav atrasts.\n";
  }

  $contextBlock .= "\n- Dokumentu veidus drīkst noteikt TIKAI no šī DB A/B bloka.\n\n";
